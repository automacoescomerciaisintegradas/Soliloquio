...
export default function Checkout() {
  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || "Nao foi possivel carregar o catalogo.");
        }
        ...
        setCatalogError(error instanceof Error ? error.message : "Erro ao carregar catalogo.");
      } finally {
  ...
  const handleCreatePayment = async () => {
    ...
    if (!emailRegex.test(email.trim())) {
      setErrorMessage("Informe um e-mail valido para gerar o PIX.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data.charge_id || !data.pix_copy_paste) {
        throw new Error(data.message || "Falha ao gerar cobranca PIX.");
      }
      ...
      <div className="bg-[radial-gradient(circle_at_top,rgba(212,175,55,0.22),transparent_45%)]">
        ...
        <section className="container pb-8 pt-4">
          ...
          <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-4" style={{ fontFamily: "Playfair Display" }}>
            Garanta Sua Colecao Completa
          </h1>
          <p className="text-lg text-gray-200 max-w-4xl">
            Os 10 volumes que transformam o monologo confuso em poder, clareza e cura emocional.
            Voce tambem pode comprar por volume ou por nome do livro, incluindo titulos de outros autores.
          </p>
        </section>
      </div>
          ...
          <div className="rounded-2xl border border-border bg-card/70 p-6">
            ...
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              <div className="rounded-xl border border-border p-4 bg-background/50">
                ...
                <p className="text-gray-300">E-mail obrigatorio para liberar sua entrega automaticamente.</p>
              </div>
              <div className="rounded-xl border border-border p-4 bg-background/50">
                <p className="text-accent font-semibold mb-1">2. Pague via PIX</p>
                <p className="text-gray-300">QR Code e copia e cola disponiveis na hora, sem redirecionamento confuso.</p>
              </div>
              <div className="rounded-xl border border-border p-4 bg-background/50">
                <p className="text-accent font-semibold mb-1">3. Acesso imediato</p>
                <p className="text-gray-300">Pagamento confirmado, botao de download liberado na propria pagina.</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card/70 p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
              ...
              <input
                ...
                placeholder="Buscar por volume, titulo ou autor"
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {[
                { id: "all", label: "Todos" },
                { id: "collection", label: "Colecao completa" },
                { id: "volume", label: "Por volume" },
                { id: "single", label: "Por titulo/autor" },
              ].map(item => (
                <button
                  key={item.id}
                  type="button"
        ...
        <aside className="lg:sticky lg:top-6 h-fit space-y-4">
          <div className="rounded-2xl border border-accent/40 bg-card p-6">
            ...
            <ul className="space-y-2 text-sm text-gray-300 mb-5">
              ...
              <li className="flex gap-2">
                <CheckCircle2 className="text-accent mt-0.5" size={16} />
                Pagamento via PIX (PushinPay) com confirmacao automatica
              </li>
              <li className="flex gap-2">
                <ShieldCheck className="text-accent mt-0.5" size={16} />
                E-mail obrigatorio para liberar entrega segura
              </li>
            </ul>
            ...
            <p className="text-xs text-gray-400 mt-3">
              Pagamento via PIX. Confirmacao automatica e liberacao sem login.
            </p>
          </div>
          ...
          {checkout && (
            <div className="rounded-2xl border border-border bg-card p-6">
              ...
              <label className="text-xs text-gray-400 block mb-1">Codigo PIX copia e cola</label>
              <textarea
      ...
      <section className="container pb-16">
        <div className="rounded-2xl border border-border bg-card/60 p-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-300">
            ...
            <div><p className="font-semibold text-accent mb-1">Preciso criar conta?</p><p>Nao. Compra direta, sem login e sem cadastro.</p></div>
            <div><p className="font-semibold text-accent mb-1">Posso comprar so um volume?</p><p>Sim. Selecione o volume individual antes de gerar o PIX.</p></div>
            <div><p className="font-semibold text-accent mb-1">Posso comprar por titulo ou autor?</p><p>Sim. Use a busca por nome para localizar rapidamente.</p></div>
            <div><p className="font-semibold text-accent mb-1">Onde recebo o link?</p><p>O botao de download aparece na mesma pagina apos o pagamento.</p></div>
            <div><p className="font-semibold text-accent mb-1">O e-mail e obrigatorio?</p><p>Sim, para validacao de entrega e suporte da compra.</p></div>
            <div><p className="font-semibold text-accent mb-1">Tem suporte?</p><p>Sim, suporte por e-mail em horario comercial.</p></div>
            <div><p className="font-semibold text-accent mb-1">Como funciona o PIX oficial?</p><p>Seguimos o padrao oficial de pagamento instantaneo do Banco Central.</p></div>
          </div>
        </div>
      </section>
